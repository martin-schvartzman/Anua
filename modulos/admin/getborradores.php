...
from nif,titulo,categoria,contenido where 
categoria.id=nif.categoria and titulo.id=nif.titulo and contenido.id=nif.contenido and
 nif.estado=0 and nif.id not in (select nif from perfil) order by fecha desc";
$borr=query($sql);
foreach($borr as $b){
